...
- **2026-04-24**: **Vault-first Obsidian KB lifecycle stabilized** — rebuilt the Obsidian project knowledge workflow around `Research/{project-slug}/`, `Sources/*`, `Results/Reports`, `02-Index.md`, and `_system/registry.md`; kept Claude Code project bindings in `.claude/project-memory/*` as a runtime layer; preserved curated index blocks; restored historical compatibility stubs; and expanded Canvas guidance for `Maps/literature.canvas`.
- **2026-04-24**: **Project-scoped Obsidian KB workflow consolidated** — replaced the older overlapping Obsidian memory skills with `obsidian-project-kb-core`, `obsidian-source-ingestion`, `obsidian-literature-workflow`, and `obsidian-kb-artifacts`, moved paper sources under `Sources/Papers/`, and made `02-Index.md` a human-readable navigation page rather than a registry mirror.
