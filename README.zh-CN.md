...
- **2026-04-24**: **Vault-first Obsidian KB 生命周期稳定化** — 将 Obsidian 项目知识库重建为 `Research/{project-slug}/`、`Sources/*`、`Results/Reports`、`02-Index.md` 与 `_system/registry.md` 这套 project-scoped 结构；保留 `.claude/project-memory/*` 作为 Claude Code runtime binding 层；保护人工维护的 index 区块；恢复旧入口兼容 stub；并补强 `Maps/literature.canvas` 的 Canvas 规则。
- **2026-04-24**: **Project-scoped Obsidian KB workflow 完成合并** — 用 `obsidian-project-kb-core`、`obsidian-source-ingestion`、`obsidian-literature-workflow`、`obsidian-kb-artifacts` 替代旧的重叠 Obsidian memory skills，将论文 source 迁入 `Sources/Papers/`，并明确 `02-Index.md` 是人类导航页，不是 registry 镜像。
