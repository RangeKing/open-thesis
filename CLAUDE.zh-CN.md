...
Claude Scholar 当前只保留一组精简的默认 agent：
- `literature-reviewer`
- `paper-miner`
- `rebuttal-writer`
- `code-reviewer`
...
这就是当前完整保留的默认集合。其他大多数工作优先走 skills 和 commands，不要默认假设还安装了更多 legacy agents。
...
- 已绑定研究仓库的知识维护 -> `obsidian-project-memory`
...
如果当前仓库已绑定 Obsidian 项目知识库，将 `obsidian-project-memory` 视为默认的长期知识维护路径。
